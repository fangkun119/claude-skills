...
5. 使用指定的编号模式
6. 输出结果到标准输出（UTF-8编码）
7. 创建足够细粒度的标题，防止任何单个最低级别标题下的文本内容过多 - 确保内容适当分布在多个子节中，而不是集中在少数几个节中
8. 所有文本内容必须放在最深、最细粒度的标题下 - 任何内容都不应直接出现在中间级别标题下，而应放在适当的最低级别子节中

## 智能分块处理机制

### 分块触发条件
- 当输入文档超过 **2000字符** 时，自动启用智能分块处理
- 每个分块大小控制在 **1500-1800字符** 以内
- 确保有足够的处理空间，同时避免过度分割
...
如果文档字符数超过2000，请启用智能分块处理模式，并说明："文档长度为X字符，将启用智能分块处理"。
如果文档不超过2000字符，请直接进行标题优化处理。
```
...
**第一步：文档分析**
```
文档长度为3500字符，将启用智能分块处理。
```

**第二步：分块分割**
```
=== 分块 1 ===
[前1700字符的内容，包含完整的语义单元]
=== 分块 1 结束 ===

=== 分块 2 ===
[后1800字符的内容，与前一个分块有50-100字符重叠]
=== 分块 2 结束 ===
```
